...
namespace MeshN {
	...
	bool ReaderWriterT<Mesh>::ogl_writer2(bool _orient, bool _smooth){////在里面把三角面片法向画出
		...
		ogl_writer(_orient, _smooth);//绘制三角网格
		HalfedgeHandle HEH;
		Mesh::FacetIterator  fit(mesh_->facet_begin());
		...
		for (; fit != mesh_->facet_end(); ++fit) {
			...
			HEH = fit->halfedge_handle_;
			FacetHandle FH = mesh_->facet_handle(HEH);
			...
			do {
				...
				glVertex3fv(mesh_->calc_centroid(FH) + mesh_->normal(FH)/5);
				HEH = mesh_->next_halfedge_handle(HEH);
			} while (HEH != fit->halfedge_handle_);
			glEnd();
		}
		return true;
