...
///////////////////三角网格去噪////////////////////////
		...
		std::cout << "数字 4 代表 实现三角网格处理" << std::endl;
		std::cout << "请输入数字(1-4): " << std::endl;
		std::cin >> choice;
		if (choice == 1)
			...
			mesh.Guided_Mesh_Normal_Filtering_2015();
		else if (choice == 4)
			mesh.mesh_process();
		std::cout << "去噪或者三角网格处理执行完毕!!!" << std::endl;
		std::cout << "按 b 可以查看去噪或者三角网格处理结果" << std::endl;
